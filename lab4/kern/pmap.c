...
mem_init(void)
{
	...
	//assert((uint32_t)kern_pgdir>0xf0000000 && (uint32_t)kern_pgdir<0xffffffff);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uint32_t lo = ROUNDDOWN((uint32_t)va, PGSIZE), hi = ROUNDDOWN((uint32_t)va + len - 1, PGSIZE);
	for(uint32_t i = lo; i <= hi; i += PGSIZE)
	{
		if((env->env_pgdir[PDX(i)] & PTE_U) && 
			((((pte_t*)UVPT)[i/PGSIZE]) & PTE_U))
			continue;
		if(i == lo)
			user_mem_check_addr = (uint32_t)va;
		else
...
check_page_free_list(bool only_low_memory)
{
    ...
    // pp2保存大于pdx_limit时候的第一个pp指针（如page_free_list），pp1保存刚进入小于XX范围时候的pp指针
	if (only_low_memory) {
